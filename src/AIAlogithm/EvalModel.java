...
public class EvalModel {
	...
	public int eval(Board board, char player) {
		for (Map.Entry<String, Piece> stringPieceEntry : board.pieces.entrySet()) {
			...
			/*
			 * The table in PiecePosition is for red player in default. To eval
			 * black player, needs to perform a mirror transformation.
			 */
			int[] reversePosition = new int[] { board.BOARD_HEIGHT - 1 - piece.currentPosition[0], piece.currentPosition[1] };
			switch (piece.getType().charAt(0)) {
			case 'b':
				if (piece.getCamp().charAt(0) == 'r')
			...
			case 'm':
				if (piece.getCamp().charAt(0) == 'r') {
					...
					values[0][1] += evalPiecePosition(3, piece.currentPosition);
				} else {
			...
			case 'j':
				if (piece.getCamp().charAt(0) == 'r') {
					...
					values[0][1] += evalPiecePosition(4, piece.currentPosition);
				} else {
			...
			case 'p':
				if (piece.getCamp().charAt(0) == 'r') {
					...
					values[0][1] += evalPiecePosition(5, piece.currentPosition);
				} else {
			...
			case 'z':
				if (piece.getCamp().charAt(0) == 'r') {
					...
					values[0][1] += evalPiecePosition(6, piece.currentPosition);
				} else {
