...
public class RookPiece extends Piece {
	...
	public void rule(Board board, String player) {
		...
		for (int offset : yOffsets) {
			...
			} else if (board.isInside(rMove) && board.getPiece(rMove).getCamp().hashCode() != player.hashCode()) {
				nextPosition.add(rMove);
				break;
			} else
				break;
		...
		for (int offset : yOffsets) {
			...
			} else if (board.isInside(lMove) && board.getPiece(lMove).getCamp().hashCode() != player.hashCode()) {
				nextPosition.add(lMove);
				break;
			} else
				break;
		...
		for (int offset : xOffsets) {
			...
			} else if (board.isInside(uMove) && board.getPiece(uMove).getCamp().hashCode() != player.hashCode()) {
				nextPosition.add(uMove);
				break;
			} else
				break;
		...
		for (int offset : xOffsets) {
			...
			} else if (board.isInside(dMove) && board.getPiece(dMove).getCamp().hashCode() != player.hashCode()) {
				nextPosition.add(dMove);
				break;
			} else
				break;
